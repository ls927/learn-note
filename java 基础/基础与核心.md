...
👀 [只重写 equals 演示代码](./test/src/HashTest.java)
👀 [重写两个方法 演示代码](./test/src/HashTest2.java)
...
👀 [intern 测试](./test/src/StringInternTest.java)
...
String ab = a + b; // 堆上的对象 等同于 StringBuilder.append(a).append(b).toString()
...
#### Java NIO
关乎 java New I/O,以及 IO 模型，另起一篇笔记详细讨论。

👁️ [a simple practice for NIO](./test/src/NIOClient.java)

👁️ [a simple practice for NIO](./test/src/NIOServer.java)
...
👀 [演示代码](./test/src/GenerateType.java)
...
- 缺点：增加了安全问题，比如可以利用反射逃避**泛型安全检查（在编译期检查）**，而在运行时没有类型的限制（**由于泛型擦除**）和 可以修改某些不可变类型如`String`。

👀 [逃避泛型安全检查演示](./test/src/UnsafeReflect.java)

👀 [改变 String 对象演示](./test/src/ReflectChangeString.java)

另一缺点是，性能的下滑，在[该测试](./test/src/ReflectPerformanceTest.java)中用反射创建对象的效率比正常创建对象的效率低了 10 倍！
...
#### 注解
`Annotation`（注解） 是 Java5 开始引入的新特性，可以看作是一种特殊的注释（写给编译器和虚拟机看的），主要用于修饰类、方法或者变量。

注解的本质是一个继承了`Annotation`的特殊接口
```
...
```
注解只有在被解析之后才能生效，常见的解析方法有两种：
- 编译期直接扫描 ：`@Retention(RetentionPolicy.SOURCE)` ，编译器在编译 Java 代码的时候扫描对应的注解并处理，比如某个方法使用`@Override` 注解，编译器在编译的时候就会检测当前的方法是否重写了父类对应的方法。
- 运行期通过反射处理 ：`@Retention(RetentionPolicy.RUNTIME)`，像框架中自带的注解(比如 Spring 框架的 @Value 、@Component)都是通过反射来进行处理的。

JDK 内置很多注解如`@PostConstruct`，还可以自定义注解
